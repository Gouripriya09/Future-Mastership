  ...
  const handleEnroll = async () => {
    try {
      ...
      if (userDocSnapshot.exists()) {
        ...
        // Check if "TRC5" is already in the courses array
        if (!coursesArray.includes("TRC5")) {
          coursesArray.push("TRC5");

          // Update the user document with the updated courses array
          await updateDoc(userDocRef, { courses: coursesArray });

          console.log("Enrolled in TRC5 successfully!");
          navigate("/dashboard");
        } else {
          console.log("Already enrolled in TRC5.");
        }
      }
    } catch (error) {
      console.error("Error enrolling in TRC5:", error.message);
    }
  };
