  ...
  const handleEnroll = async () => {
    try {
      ...
      if (userDocSnapshot.exists()) {
        ...
        // Check if "TRC1" is already in the courses array
        if (!coursesArray.includes("TRC1")) {
          coursesArray.push("TRC1");

          // Update the user document with the updated courses array
          await updateDoc(userDocRef, { courses: coursesArray });

          console.log("Enrolled in TRC1 successfully!");
          navigate("/dashboard");
        } else {
          console.log("Already enrolled in TRC1.");
        }
      }
    } catch (error) {
      console.error("Error enrolling in TRC1:", error.message);
    }
  };
